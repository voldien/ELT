...
ExTexture* ExCreateTexture(ExTexture* texture, unsigned int target, int level,
	...
	case GL_TEXTURE_2D_ARRAY:
		glTexStorage3D(GL_TEXTURE_2D_ARRAY, level, texture->internalformat,
				width, height, texture->layer);
		break;
...
void ExSubTexture(ExTexture* texture, int level, int xoffset, int yoffset,
		int width, int height, int format, int type, const void *pixels) {
	glBindTexture(texture->target, texture->texture);
	switch (texture->target) {
	case GL_TEXTURE_2D:
		glTexSubImage2D(texture->target, level, xoffset, yoffset, width, height,
				format, type, pixels);
		break;
#ifndef GL_ES_VERSION_2_0
	case GL_TEXTURE_1D:
		glTexSubImage1D(texture->target, level, xoffset, width, format, type,
				pixels);
		break;
	case GL_TEXTURE_2D_ARRAY:
		glTexSubImage3D(texture->target, level, xoffset, yoffset, 0, width,
...
unsigned int ExGetTextureLevelDatai(unsigned int textureid, unsigned int level,
