...
ExFrameBuffer* ExCreateRenderTexture2D(ExFrameBuffer* framebuffer,
	...
	ExCreateTexture(&framebuffer->texture, GL_TEXTURE_2D, 0, GL_RGB, width,
			height, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);

	glFramebufferTexture2D(GL_FRAMEBUFFER, attachment,
			framebuffer->texture.target, framebuffer->framebuffer, 0);

	glDrawBuffers(1, &attachment);
	State = glCheckFramebufferStatus(GL_FRAMEBUFFER);
	if (State != GL_FRAMEBUFFER_COMPLETE) {
		printf("Failed to Initilize ForwardRendering FrameBuffer! | %d",
				glGetError());
		return NULL;
	}
