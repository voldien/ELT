...
void ExSignalCatch(int signal) {
	...
	switch (signal) {
	case SIGSEGV:
		ExSPrintf(wchar, EX_ERROR_MESSAGE, app_name,
				EX_TEXT("Error : segment violation.\n"));
		break;
	case SIGINT:
		ExSPrintf(wchar, EX_ERROR_MESSAGE, app_name,
				EX_TEXT("Error : interrupt.\n"));
		exit(1);
		break;
	case SIGILL:
		ExSPrintf(wchar, EX_ERROR_MESSAGE, app_name,
				EX_TEXT(
						"Error : illegal instruction - invalid function image.\n"));
		break;
	case SIGFPE:
		ExSPrintf(wchar, EX_ERROR_MESSAGE, app_name,
				EX_TEXT("Error : floating point exception.\n"));
		break;
	case SIGTERM:
		ExSPrintf(wchar, EX_ERROR_MESSAGE, app_name,
				EX_TEXT("Error : Software termination signal from kill.\n"));
		break;
	case SIGABRT:
		ExSPrintf(wchar, EX_ERROR_MESSAGE, app_name,
				EX_TEXT(
						"Error : abnormal termination trigged by abort call.\n"));
		break;
#if defined(EX__UNIX)
		case SIGQUIT:
