...
ExEGLContext ExCreateEGLContext(ExWin window, ExEGLContext shared) {
	...
	/*	Choose Config	*/
	if ((hr = eglChooseConfig(eglDisplay, configAttribList, &eglConfig, 1,
			&numConfig)) != EGL_TRUE) {
		ExError(EX_TEXT("failed to Choose config for EGL.\n"));
	}

	if (!(eglSurface = eglCreateWindowSurface(eglDisplay, eglConfig,
	...
	if ((hr = eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext))
			!= EGL_TRUE)
...
int ExMakeEGLCurrent(EX_RESTRICT ExWindowContext hDC,
		EX_RESTRICT ExEGLContext glc) {
