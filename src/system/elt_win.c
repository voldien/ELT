...
ExChar* ExGetDefaultWindowTitle(ExChar* text, int length) {
	...
	sprintf(wchar, EX_ENGINE_VERSION_STRING,
			...
			EX_VERSION_MINOR,
			EX_VERSION_REVISION,
			EX_ENGINE_STATUS, "",		//ExGetOSName(),
			major, minor);
	memcpy(text, wchar,
			EX_MAX(sizeof(ExChar) * length + sizeof(ExChar),
					sizeof(wchar) / sizeof(ExChar)));
	return text;
}
...
ExWin ExCreateWindow(int x, int y, int width, int height, unsigned int flag) {
	...
	ExHandle handle;
	char title[256];
	typedef ExOpenCLContext (*pExCreateCLSharedContext)(ExOpenGLContext glc,
			ExWindowContext window, unsigned int flag);
	pExCreateCLSharedContext opencl;
	unsigned int tmpCore = ExOpenGLGetAttribute(EX_OPENGL_CONTEXT_PROFILE_MASK,
			NULL);
	handle = ExLoadObject("");
	if (flag & EX_OPENCL) {
	...
	if ((flag & EX_OPENGL_CORE) == EX_OPENGL_CORE) {
		ExOpenGLSetAttribute(EX_OPENGL_CONTEXT_PROFILE_MASK,
				EX_GL_CONTEXT_PROFILE_CORE);
	}
