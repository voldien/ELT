...
/**/
extern int ExGetOpenCLDevice(cl_platform_id platform, cl_device_id* device,
		unsigned int flag);
...
static char* private_get_device_extension(cl_device_id device) {
	...
	clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, extension_size, extension,
			NULL);
	return extension;
}
...
ExOpenCLContext ExCreateCLContext(unsigned int flag, unsigned int platform) {
	...
	ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform, CL_DEVICE_TYPE_GPU,
			0, NULL, &uiGPUDevCount);
	ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform, CL_DEVICE_TYPE_CPU,
			0, NULL, &uiCPUDevCount);
	cdDevices = (cl_device_id*) malloc(
			sizeof(cl_device_id) * (uiCPUDevCount + uiGPUDevCount));
	if (uiGPUDevCount) {
		ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform,
				CL_DEVICE_TYPE_GPU, uiGPUDevCount, &cdDevices[0], NULL);
		numDevices += uiGPUDevCount;
	}

	if (uiCPUDevCount) {
		ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform,
				CL_DEVICE_TYPE_CPU, uiCPUDevCount, &cdDevices[uiGPUDevCount],
				NULL);
	...
	hClContext = clCreateContext(props, numDevices, cdDevices, NULL, NULL,
			&ciErrNum);
...
ExOpenCLContext ExCreateCLSharedContext(ExOpenGLContext glc, unsigned int flag) {
	...
	if (!(ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform,
			CL_DEVICE_TYPE_GPU, 0, NULL, &uiDevCount))) {
		// create OpenCL Devices on the GPU
		cdDevices = (cl_device_id*) malloc(sizeof(cl_device_id) * uiDevCount);
		ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform,
				CL_DEVICE_TYPE_GPU, uiDevCount, cdDevices, NULL);
		size += uiDevCount;
	}

	/* Get Device ID from CPU	*/
	else if (!(ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform,
			CL_DEVICE_TYPE_CPU, 0, NULL, &uiDevCount))) {
		// create OpenCL Devices on the CPU
		cdDevices = (cl_device_id*) realloc(cdDevices,
				size + sizeof(cl_device_id) * uiDevCount);
		ciErrNum = clGetDeviceIDs((cl_platform_id) cpPlatform,
				CL_DEVICE_TYPE_CPU, uiDevCount, &cdDevices[size], NULL);
	}
...
#	endif 
	...
	if(layer & EX_OPENGL || erenderingFlag & EX_EGL) {props[2] = CL_EGL_DISPLAY_KHR;}
		...
		props[2] = CL_CGL_SHAREGROUP_KHR;
	} else if (flag & EX_EGL) {
		props[2] = CL_EGL_DISPLAY_KHR;
	}

	/*	Create OpenCL context.	*/
	hClContext = clCreateContext(props, size - 1, cdDevices, NULL, NULL,
			&ciErrNum);
	if (!hClContext || ciErrNum != CL_SUCCESS) {
		ExDevPrint(
				"Failed to Create OpenCL Context based on the OpenGL Context");
	}
...
ERESULT ExQueryCLContext(ExOpenCLContext context, unsigned int param_name,
		ExHandle param_value, unsigned int* num) {
	cl_int ciErrNum;
	size_t size;
	if (!context)
		return -1;
	...
	case CL_CONTEXT_DEVICES: {/* Get Context Device  */
		...
		if (num)
			*num = num_dev;
		if (param_value)
			ciErrNum = clGetContextInfo((cl_context) context,
					CL_CONTEXT_DEVICES, num_dev * sizeof(cl_device_id),
					param_value, &size);
	}
		break;
	case CL_CONTEXT_INTEROP_USER_SYNC: { /**/
		ciErrNum = clGetContextInfo((cl_context) context,
				CL_CONTEXT_INTEROP_USER_SYNC, sizeof(cl_uint), param_value,
				&size);
	}
		break;
	case CL_CONTEXT_NUM_DEVICES: /**/
		ciErrNum = clGetContextInfo((cl_context) context,
				CL_CONTEXT_NUM_DEVICES, sizeof(cl_uint), param_value, &size);
		break;
	case CL_CONTEXT_PROPERTIES:
		ciErrNum = clGetContextInfo((cl_context) context, CL_CONTEXT_PROPERTIES,
				NULL, NULL, &size);
		ciErrNum = clGetContextInfo((cl_context) context, CL_CONTEXT_PROPERTIES,
	...
	case CL_CONTEXT_REFERENCE_COUNT: {
		...
		ciErrNum = clGetContextInfo((cl_context) context,
				CL_CONTEXT_REFERENCE_COUNT, sizeof(cl_uint), param_value,
				&size);
	...
	case CL_CONTEXT_PLATFORM:
		ciErrNum = clGetContextInfo((cl_context) context, CL_CONTEXT_PLATFORM,
				sizeof(cl_platform_id), param_value, &size);
		break;
	case CL_CURRENT_DEVICE_FOR_GL_CONTEXT_KHR: {
		typedef cl_int (CL_API_CALL*clGetGLContextInfoKHR)(
				const cl_context_properties * /* properties */,
				cl_gl_context_info /* param_name */,
				size_t/* param_value_size */, void */* param_value */,
				size_t */* param_value_size_ret */);
		cl_context_properties con_prop[10] = { 0 };
		clGetGLContextInfoKHR func =
				(clGetGLContextInfoKHR) clGetExtensionFunctionAddress(
						"clGetGLContextInfoKHR");

		ExQueryCLContext(context, &con_prop, CL_CONTEXT_PROPERTIES, NULL);

		ciErrNum = func(con_prop, CL_CURRENT_DEVICE_FOR_GL_CONTEXT_KHR,
				sizeof(cl_device_id), param_value, &size);
	}
		break;
...
int ExGetOpenCLPlatforms(unsigned int num_entries, ExCLPlatformID* platforms,
		unsigned int* num_platform) {
	return clGetPlatformIDs(num_entries, platforms, num_platform);
}

int ExGetOpenCLDevices(ExCLPlatformID platform, unsigned int type,
		unsigned int num_entries, ExCLDeviceID* devices,
		unsigned int* num_devices) {
	return clGetDeviceIDs(platform, type, num_entries, devices, num_devices);
}

int ExGetContextDevices(ExOpenCLContext context, ExCLDeviceID* devices,
		unsigned int* num) {
	cl_uint numDevices;
	clGetContextInfo(context, CL_CONTEXT_NUM_DEVICES, sizeof(numDevices),
			&numDevices, NULL);

	if (devices) {
		clGetContextInfo(context, CL_CONTEXT_NUM_DEVICES,
				numDevices * sizeof(ExCLDeviceID), devices, NULL);
	}
	if (num) {
		*num = numDevices;
	}
...
int ExGetCLPlatformID(int* clSelectedPlatformID, unsigned int* num,
		unsigned int flag) {
	char chBuffer[1024];
	...
	*clSelectedPlatformID = NULL;
	...
	if (ciErrNum != CL_SUCCESS) {
		...
		if (num_platforms == 0) {
			...
			/*	*/
			ciErrNum = clGetPlatformIDs(num_platforms, &temp_clPlatformIDs[0],
					NULL);
			if (ciErrNum == CL_DEVICE_NOT_FOUND)
				ExDevPrint("Couldn't find");

			/*	get number of gpu device.s	*/
			ciErrNum = clGetDeviceIDs(temp_clPlatformIDs[0], CL_DEVICE_TYPE_GPU,
					0, NULL, &num_gpu);

			/*	get number of cpu devices.	*/
			ciErrNum = clGetDeviceIDs(temp_clPlatformIDs[0], CL_DEVICE_TYPE_CPU,
					0, NULL, &num_cpu);

			if ((clPlatformIDs = (cl_platform_id*) malloc(
					num_platforms * sizeof(cl_platform_id))) == NULL) {
			...
			for (i = 0; i < num_platforms; ++i) {
				...
				clGetDeviceIDs(clPlatformIDs[i], CL_DEVICE_TYPE_ALL, 0, 0,
						&num_device);
				ciErrNum = clGetDeviceIDs(clPlatformIDs[i], CL_DEVICE_AVAILABLE,
						num_device, &device_id[0], NULL);
				...
				ciErrNum = clGetDeviceIDs(clPlatformIDs[i], CL_DEVICE_TYPE_GPU,
						1, &device_id[0], NULL);
...
ExCLCommandQueue ExCreateCommandQueue(ExOpenCLContext context,
		ExCLDeviceID device) {
	cl_int errNum;
	...
	commandQueue = clCreateCommandQueue((cl_context) context,
			(cl_device_id) device, 0, &errNum);
...
ExCLProgram ExCreateProgram(ExOpenCLContext context, ExCLDeviceID device,
		const ExChar* cfilename, ...) {
	va_list list;
	cl_int errNum = 0;
	cl_program program;
	char* clsourcecode;
	...
	program = clCreateProgramWithSource(context, 1,
			(const char**) &clsourcecode, NULL, &errNum);
	if (errNum != CL_SUCCESS)
		goto error;

	if (device == NULL)
		ExQueryCLContext(context, CL_CONTEXT_DEVICES, devices, &numDevices);
	errNum = clBuildProgram(program, numDevices, devices,
			numArg > 0 ? &buffer[0] : NULL, NULL, NULL);

	error: if (errNum != CL_SUCCESS) {
		int status;
		char buildLog[1024];
...
ExCLMem ExCreateCLBuffer(ExOpenCLContext context, unsigned int flag, int size,
		void* host_ptr) {
	cl_int error;
...
static int private_ExCLHighestFLOPS(int* clSelectedPlatformID) {
	...
	clGetDeviceIDs((cl_platform_id) clSelectedPlatformID[i], CL_DEVICE_TYPE_ALL,
			0, 0, &num_device);
...
#define CL_DEVICE_OPENCL_C_VERSION 0x103D
	...
	cl_uint addr_bits;
	clGetDeviceInfo(device, CL_DEVICE_ADDRESS_BITS, sizeof(addr_bits),
			&addr_bits, NULL);
	...
	if (nv_device_attibute_query) {
		...
		clGetDeviceInfo(device, CL_DEVICE_REGISTERS_PER_BLOCK_NV,
				sizeof(cl_uint), &regs_per_block, NULL);
		...
		clGetDeviceInfo(device, CL_DEVICE_INTEGRATED_MEMORY_NV, sizeof(cl_bool),
				&integrated_memory, NULL);
	...
	ExPrintf("CHAR %u, SHORT %u, INT %u, LONG %u, FLOAT %u, DOUBLE %u\n\n\n",
			vec_width[0], vec_width[1], vec_width[2], vec_width[3],
			vec_width[4], vec_width[5]);
...
int ExGetClDevCap(void* device) {
	...
	clGetDeviceInfo((cl_device_id) device, CL_DEVICE_EXTENSIONS,
			sizeof(cDevString), &cDevString, NULL);
	if (cDevString != 0) {
		//string stdDevString;
	...
	if (bDevAttributeQuery) {
		...
		clGetDeviceInfo((cl_device_id) device,
				CL_DEVICE_COMPUTE_CAPABILITY_MAJOR_NV, sizeof(cl_uint),
				(void*) &iComputeCapMajor, NULL);
		clGetDeviceInfo((cl_device_id) device,
				CL_DEVICE_COMPUTE_CAPABILITY_MINOR_NV, sizeof(cl_uint),
				(void*) &iComputeCapMinor, NULL);
...
static char* ExGetCLErrorMessage(cl_int error) {
#ifdef EX_DEBUG
	static const char* errorString[] = { "CL_SUCCESS", "CL_DEVICE_NOT_FOUND",
			...
			"CL_INVALID_ARG_INDEX", "CL_INVALID_ARG_VALUE",
