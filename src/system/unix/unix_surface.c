...
void ExDisplaySurfaceToWindow(ExWin window, ExSurface surface) {
	...
	if (ExGetSurfaceRect(surface, &rect)) {
		...
		XPutImage(display, window, gc, surface, 0, 0, 0, 0, rect.width,
				rect.height);
	}
}

ERESULT ExResizeSurface(ExSurface surface, unsigned int width, unsigned height) {
	((XImage*) surface)->data = realloc(((XImage*) surface)->data,
			width * height * ((XImage*) surface)->bitmap_pad);
...
int ExSetSurfacePixel(void* surface, unsigned int x, unsigned int y,
		unsigned long pixel) {
