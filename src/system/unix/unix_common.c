...
int ExCreateProcessl(const ExChar* applicationName, ...) {
	...
	case 0: {
		...
		if (execv(applicationName,
				(const char*) applicationName + sizeof(void*)) == -1)
			fprintf(stderr, strerror(errno));
	}
		break;
	default: {
...
int ExSetScreenSize(int index, int width, int height) {
	...
	XRRSetCrtcConfig(display, sr, NULL, CurrentTime, ci->x, ci->y, bestMode,
...
#   if defined(EX_GNUC) || defined(EX_GNUC)
	extern char* __progname;
	memcpy(name,/*program_invocation_name*/__progname,
			strlen(__progname) + 1 < length ? strlen(__progname) + 1 : length);
	return name;
#   else
	extern char* __progname;
...
unsigned long ExGetTotalUsedVirtualMemory(void) {
	...
	return ((sys_info.totalram - sys_info.freeram)
			+ (sys_info.totalswap - sys_info.freeswap)) * sys_info.mem_unit;
}
...
ExChar* ExGetClipboardText(void) {
	...
	XGetWindowProperty(display, owner, selection, 0, INT_MAX / 4, False, format,
			&seln_type, &seln_format, &nbytes, &overflow, &src);
