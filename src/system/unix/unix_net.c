...
ExSocket ExCreateSocket(unsigned int domain, unsigned int style,
...
int ExSetSocketOption(ExSocket socket, unsigned int level, unsigned int option,
		void* pvalue, unsigned int optlen) {
...
ExSocket ExSetSocketSendTimeOut(ExSocket socket, long int nanosec) {
	...
	return ExSetSocketOption(socket, SOL_SOCKET, SO_SNDTIMEO, &time,
			sizeof(time));
}
...
ExSocket ExSetSocketRecvTimeOut(ExSocket socket, long int nanosec) {
	...
	time.tv_sec = 0;
	time.tv_usec = nanosec;
	return ExSetSocketOption(socket, SOL_SOCKET, SO_RCVTIMEO, &time,
			sizeof(time));
}
...
long int ExRecvFrom(ExSocket socket, void* buffer, int len, unsigned int flags,
		ExSocket* from, int* fromlen) {
	return recvfrom(socket, buffer, len, flags, ((struct sockaddr*) from),
			fromlen);
}

long int ExSendTo(ExSocket socket, void* buffer, int len, unsigned int flags,
		ExSocket* to, int tolen) {
...
ExChar* ExGetinterfaceAddr(const ExChar* interface, ExChar* addr,
		unsigned int len) {
	ExSocket fd;
	struct ifreq ifr;
	...
	unsigned int inetlen = strlen(
			inet_ntoa(((struct sockaddr_in*) &ifr.ifr_addr)->sin_addr));
	memcpy(addr, inet_ntoa(((struct sockaddr_in*) &ifr.ifr_addr)->sin_addr),
