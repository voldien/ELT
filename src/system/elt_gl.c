...
#elif defined(EX_LINUX)
	return glXMakeCurrent(display, (GLXDrawable) drawable,
			(ExOpenGLContext) glc);
#elif defined(EX_ANDROID)
...
#ifndef EX_ANDROID
	return (unsigned int) (atof(
			(const char*) glGetString(GL_SHADING_LANGUAGE_VERSION)) * 100.0f);
#else
	return 0;
#endif
}
...
unsigned int ExGetOpenGLVendor(void) {
	if (ExIsVendorNvidia())
		return EX_NVIDIA;
	else if (ExIsVendorAMD())
		return EX_AMD;
	else if (ExIsVendorintel())
		return EX_INTEL;
	else
		return EX_GPU_VENDOR_UNKNOWN;
}
