...
DECLSPEC HANDLE ELTAPIFASTENTRY ExGetD3DContext(void){
	return d3d;
}

DECLSPEC HANDLE ELTAPIFASTENTRY ExGetDirect3DContext(void){
	return directDevice;
}
...
DECLSPEC ExDevice9D ELTAPIENTRY ExCreateDirectXContext(ExWin window){
	...
	/*	check if directX modular is loaded onto the library.	*/
	...
	if(FAILED(hresult = d3d->GetDeviceCaps(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, &m_d3dDevCaps))){
		ExErrorLog(EX_TEXT("Failed to Get DirectX 9 developer compadbility"), ExGetHResultErrorMessage(hresult));
	}
